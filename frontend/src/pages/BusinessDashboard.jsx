...
export default function BusinessDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const coinTxRes = await api.get('/api/coin-transactions/');
      if (coinTxRes.status === 200) {
        setCoinTransactions(coinTxRes.data.results || coinTxRes.data);
      }
    } catch (err) {
    ...
    <div className="container py-5">
      ...
      <div className="d-flex flex-wrap gap-2 mb-4">
        ...
        <button 
          ...
          className={`neo-btn py-2 px-4 ${activeTab === 'coins' ? 'active font-weight-bold border-dark' : ''}`}
        >
          <i className="bi bi-coin me-2"></i> ULU Coins
        </button>
      </div>
      ...
      {activeTab === 'coins' && (
        <div>
          {/* Coin Balance Bar */}
          <NeomorphicCard className="p-4 d-flex align-items-center justify-content-between mb-4">
