...
export default function UserProfileHeader({ profile, subscription, handleToggleOnline }) {
  ...
  const handleImageUpload = async (e, type) => {
    ...
    try {
      // Don't set Content-Type manually — axios auto-detects the
      // multipart/form-data boundary from FormData objects.
      ...
      console.error(`Upload ${type} error:`, err.response?.data || err);
      alert(`Failed to upload ${type}. ${err.response?.data?.detail || ''}`);
    }
  };
  ...
  return (
    <NeomorphicCard className="mb-5 overflow-hidden p-0 position-relative" elevation="convex">
      {/* Banner Section */}
      <div className="bg-light position-relative" style={{ height: '250px', width: '100%', cursor: 'pointer' }} onClick={() => bannerInputRef.current.click()}>
        {user.banner ? (
          <img src={user.banner} alt="Banner" className="w-100 h-100" style={{ objectFit: 'cover' }} />
        ) : (
          <div className="w-100 h-100 bg-secondary opacity-25 d-flex align-items-center justify-content-center">
            <i className="bi bi-camera fs-1 text-dark opacity-50"></i>
          </div>
        )}
        <div className="position-absolute bottom-0 end-0 p-3">
          <span className="badge bg-dark opacity-75"><i className="bi bi-camera me-1"></i> Edit Cover</span>
        </div>
        <input type="file" accept="image/*" className="d-none" ref={bannerInputRef} onChange={(e) => handleImageUpload(e, 'banner')} />
      </div>
      ...
      <div className="px-4 pb-4 position-relative" style={{ marginTop: '-60px' }}>
        <div className="d-flex flex-column flex-md-row align-items-center align-items-md-end justify-content-between mb-4">
          ...
          {/* Avatar Section */}
          <div className="text-center text-md-start mb-3 mb-md-0 position-relative z-1" style={{ cursor: 'pointer' }} onClick={() => avatarInputRef.current.click()}>
